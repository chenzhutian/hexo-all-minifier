...
function concatJS(data) {
  ...
  return Promise.all(routeList.filter(path => path.endsWith('.html')).map(path => {
    ...
    .then(() => {
      ...
      for (const path of htmlPaths) {
        ...
        for (let i = srcs.length - 1; i >= 0; --i) { // reverse for deletion
          ...
          if (scripts.indexOf(src) === -1) {
            ...
            if (count > 1) {
              // remove
              (options.silent ? log.debug : log.info)('update Concate JS: remove %s from %s', src, path);
      ...
      if (scripts.length > 1) {
        ...
        for (const path of htmlPaths) {
          ...
          (options.silent ? log.debug : log.info)('update Concate JS: add /%s to %s', route.format(bundlePath), path);
        }
      }
    ...
    .then((scripts) => {
      ...
      (options.silent ? log.debug : log.info)('update Concate JS: try to concat %s scripts', scripts.length);
      return Promise.all(scripts.map(path => {
        return new Promise((resolve, reject) => {
          ...
          script.on('end', () => {
            route.remove(path);
            (options.silent ? log.debug : log.info)('update Concate JS: concat script %s', path);
            resolve(scriptTxt);
          });
        });
      })).then(results => {
        ...
        (options.silent ? log.debug : log.info)('update Concate JS: finish concat js script');
      });
    });
}
